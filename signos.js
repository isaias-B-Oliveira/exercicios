...
const verifica_data_range = (data, data_inicio, data_fim, data_comparacao)=>{
    if(data_comparacao == "and"){
        return(data >= data_inicio && data <= data_fim);
    }else if(data_comparacao == "or"){
        return(data >= data_inicio || data <= data_fim);
    }
}
...
const retorna_signo =(signos, data)=>{
    ...
    for (const signo of signos){
        ...
        let data_comparacao = signo["DataInicio"] == "12-22" ? "or" : "and" ;

        if(verifica_data_range
             (data, data_inicio_signo, data_fim_signo, data_comparacao)){
                return signo["Nome"];
            }
    }
}
...
console.log("O signo de hoje e:" + nome_signos);